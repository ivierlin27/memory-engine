...
- **Sync chat models from LM Studio**: On the LXC, from the compose project directory (e.g. `/opt/memory-engine`), run `./scripts/sync-khoj-chat-models.sh`. The script is piped into `docker compose exec` (no bind-mount required). It calls `GET /v1/models` using `OPENAI_BASE_URL` / `OPENAI_API_KEY` from compose and upserts Khoj **ChatModel** rows under **Ai Model API** `LM Studio (synced)`. Optional: `--dry-run`, `--prune` (only removes unreferenced models).
